...
public class Parser {
	...
	public const int equalequal_Sym = 10;
	public const int bangequal_Sym = 11;
	public const int less_Sym = 12;
	public const int lessequal_Sym = 13;
	public const int greater_Sym = 14;
	public const int greaterequal_Sym = 15;
	public const int plus_Sym = 16;
	public const int minus_Sym = 17;
	public const int star_Sym = 18;
	public const int slash_Sym = 19;
	public const int percent_Sym = 20;
	public const int bang_Sym = 21;
	public const int true_Sym = 22;
	public const int false_Sym = 23;
	public const int lparen_Sym = 24;
	public const int rparen_Sym = 25;
...
// List keeps track of the symbol table
	...
	static void Expression(out int ExprValue) {
		...
		while (la.kind == barbar_Sym) {
			...
			ExprValue = ToInt(ExprValue || ExprAlsoValue);
		}
	}
	...
	static void AndExp(out int andExpConst) {
		...
		while (la.kind == andand_Sym) {
			...
			andExpConst = ToInt(ToBool(andExpConst) && ToBool(andExpAlsoConst));
		}
	}

	static void EqlExp(out int equalExpConst) {
		int equalExpAlsoConst;
		...
		while (la.kind == equalequal_Sym || la.kind == bangequal_Sym) {
			if (la.kind == equalequal_Sym) {
				...
				equalExpConst = ToInt(equalExpConst == equalExpAlsoConst);
			} else {
				Get();
				RelExp(out equalExpAlsoConst);
				equalExpConst = ToInt(equalExpConst != equalExpAlsoConst);
	...
	static void RelExp(out int relExpConst) {
		...
		if (StartOf(3)) {
			if (la.kind == less_Sym) {
				...
				relExpConst = ToInt(relExpConst < relExpAlsoConst);
	...
	static void Factor(out int factorNum) {
		if (la.kind == identifier_Sym) {
			Variable();
			factorNum = ValueOf(sym.val);
		} else if (la.kind == number_Sym) {
			Number();
			factorNum = Convert.ToInt32(sym.val);
		} else if (la.kind == true_Sym) {
			Get();
			factorNum = ToInt(true);
		} else if (la.kind == false_Sym) {
			Get();
			factorNum = ToInt(false);
	...
	static bool[,] set = {
		...
		{x,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, T,T,x,x, x,T,T,T, T,x,x,x},
		{x,x,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, T,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x},
		{x,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,x,x,x}
...
public class Errors {
	...
	public static void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 29: s = "invalid UnaryExp"; break;
